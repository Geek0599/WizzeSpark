(() => {
    ...
    function tableSlider() {
        const sliderBlock = document.querySelector("[data-table-slider]");
        if (sliderBlock) {
    ...
    observeElements([ {
        ...
        element: document.querySelector("#support-image"),
        callback: () => {
            document.querySelector("#letter").classList.add("_anim");
        },
        offset: `${parseOffset("-5%")}px 0px`
    } ]);
    ...
    function showMoreHideGridElems() {
        ...
        if (wrapperBlocks.length) wrapperBlocks.forEach((block => {
            ...
            if (block) {
                ...
                window.addEventListener("resize", (() => setTimeout((() => {
                    debouncedSetMaxHeight(gridContainer, gridItems, row, btnShowHide, savedWidth);
                }), 50)));
        ...
        function calculateHeight(gridContainer, gridItems, row, memoryHeight) {
            if (memoryHeight) return gridContainer.getAttribute("data-initial-height");
            ...
            let totalHeight = 0;
            for (let i = 0; i < rowOnMaxWidth; i++) totalHeight += gridItems[i].offsetHeight + (rowGap ? rowGap : 0);
            totalHeight -= rowGap ? rowGap : 0;
            gridContainer.setAttribute("data-initial-height", totalHeight);
            return totalHeight;
        }
        function setMaxHeight(gridContainer, gridItems, row, btnShowHide, savedWidth, memoryHeight) {
            if (savedWidth && window.innerWidth == savedWidth) return; else {
                savedWidth = window.innerWidth;
                const height = calculateHeight(gridContainer, gridItems, row, memoryHeight);
        ...
        function toggleHeight(btnShowHide, gridContainer, gridItems, row, wrapperBlock) {
            function toggle(btnShowHide, gridContainer, gridItems, row) {
                ...
                if (!isOpen) {
                    ...
                    setMaxHeight(gridContainer, gridItems, row, btnShowHide, false, true);
                    btnShowHide.classList.remove("_extended");
                }
                if (toggleText) {
            ...
            if (btnShowHide.hasAttribute("data-table-pc")) if (!tablePc.classList.contains("_expand")) {
                tablePc.classList.add("_expand");
                tablePc.classList.toggle("_details");
                toggle(btnShowHide, gridContainer, gridItems, row);
            } else {
                tablePc.classList.remove("_expand");
                toggle(btnShowHide, gridContainer, gridItems, row);
                window.setTimeout((() => {
                    tablePc.classList.toggle("_details");
                }), 500);
    ...
    const tableMb = document.querySelector(".table-mb");
    if (tablePc && tableMb) {
        window.addEventListener("resize", (() => {
            tablePc.classList.remove("_expand", "_details");
            tablePc.dispatchEvent(new CustomEvent("resizeOpenTable", {
                bubbles: true
            }));
        }));
    ...
    magnetContainers.forEach((container => {
        ...
        switch (oval.getAttribute("data-magnet-bg")) {
          case "oval":
            calcParams.x = window.innerWidth > 768 ? -51 : -50;
            calcParams.rotate = "-9deg";
            break;

          case "oval-lg":
            calcParams.y = -46;
            calcParams.rotate = window.innerWidth > 768 ? "-9deg" : "-43deg";
            break;
        }
        const applyTransform = (offsetX, offsetY) => {
            ...
            oval.style.transform = `translate3d(${calcParams.x + ovalShiftX / oval.offsetWidth * 100}%, ${calcParams.y + ovalShiftY / oval.offsetHeight * 100}%,0) ${calcParams.rotate ? "rotate(" + calcParams.rotate + ")" : ""}`;
            link.style.transform = `translate(${linkShiftX}px, ${linkShiftY}px)`;
        };
        const resetTransform = () => {
            oval.style.transform = `translate3d(${calcParams.x}%, ${calcParams.y}%, 0) ${calcParams.rotate ? "rotate(" + calcParams.rotate + ")" : ""}`;
